...
class PaymentsServiceTest {
	...
	public void setUp() throws IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException {
		...
		paymentsService = spy(new PaymentsService(ciRepository, serviceRepository));
		...
		containerResponse = client.getDatabase("db").createContainerIfNotExists("services", "/transferCategory").block();
		Assertions.assertEquals(201, containerResponse.getStatusCode());
	...
	@Test
	void createSpontaneousPayment() {
		assertTrue(emulator.isRunning());
		PaymentPositionModel ppm = paymentsService.createSpontaneousPayment("organizationTest", TestUtil.getSpontaneousPaymentModel());
		assertEquals(null , ppm.getFiscalCode());
		assertEquals(null , ppm.getFullName());
	}
	...
	void createSpontaneousPayment_404() {
		...
		try {
			...
			paymentsService.createSpontaneousPayment("organizationFake", TestUtil.getSpontaneousPaymentModel());
			fail();
		...
		try {
			...
			paymentsService.createSpontaneousPayment("organizationTest", TestUtil.getSpontaneousPaymentModel_FakeServiceId());
			fail();
		} catch (AppException e) {
			assertEquals(HttpStatus.NOT_FOUND, e.getHttpStatus());
		} catch (Exception e) {
		...
		try {
			// non-existent enrollment between organization ad service -> must raise a 404 exception
			paymentsService.createSpontaneousPayment("organizationTest", TestUtil.getSpontaneousPaymentModel_NotEnrolledService());
			fail();
		} catch (AppException e) {
			assertEquals(HttpStatus.NOT_FOUND, e.getHttpStatus());
		} catch (Exception e) {
	...
	void createSpontaneousPayment_400() {
		assertTrue(emulator.isRunning());
		try {
			// non-existent in request a configured property in service  -> must raise a 400 exception
			paymentsService.createSpontaneousPayment("organizationTest", TestUtil.getSpontaneousPaymentModel_BadProperty());
			fail();
		} catch (AppException e) {
